...
function BettingPanel({ bankroll, onBet, defaultBet = 0, showHotkeys = true, betMode = 'fixed' }) {
  ...
  const chipValues = isPercentMode ? PERCENT_BETS : QUICK_BETS;
...
export default function MultiplayerTable({ gameState, playerId, send, onLeave, volumeOn, onVolumeChange, volumeLevel = 1, onVolumeLevelChange, rebetEnabled = true, onRebetChange, showHotkeys = true, onShowHotkeysChange, betMode = 'fixed', onBetModeChange, onApproveJoin, onRemoveSpectator, onResetLobby }) {
          ...
          {isSpectating ? (
            ...
            {isLocalBetting && (
              <div className="betting-controls">
                <BettingPanel bankroll={localPlayer.bankroll} onBet={handleBet} defaultBet={rebetEnabled ? lastBetAmount : 0} showHotkeys={showHotkeys} betMode={betMode} />
              </div>
            )}
